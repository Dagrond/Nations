...
public class NationMember {
  ...
  /*
   * Permissions:
   * - jail - player can jail others player
   * - kick - player can kick others from nation
   * - ban - player can ban player from nation (nation recognises banned player as enemy, even if he's without nation)
   * - permission_view - player can view permission of other players
   * - permission_manage - player can manage permissions of nations members that are lower priority than himself
   * todo
   */
  ...
  public void setNation(Nation nation) {
    priority = 0;
    permissions.clear();
    if (this.nation != null) this.nation.delMember(player);
    this.nation = nation;
  ...
  public static NationMember getNationMember(UUID player) {
    for (NationMember member : members) {
      if (player.equals(member.getUUID())) return member;
    }
