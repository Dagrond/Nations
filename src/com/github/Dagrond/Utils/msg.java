	...
	public static String get(String path, boolean prefix, String... opt) {
		if (!msg.isString(path))
			return ChatColor.RED
					+ "Error occured during loading message. Perhaps Messages.yml is configured badly. Please, contact server operator.";
		String string = "";
		if (prefix)
		...
		if (path.startsWith("error_")) {
			...
			if (opt.length > 0) {
				for (int i = 0; i < opt.length; i++) {
					string = string.replaceAll("%" + (i + 1),
							msg.getString("error_variable_color") + opt[i] + msg.getString("error_color"));
				}
			}
			...
			if (opt.length > 0) {
				for (int i = 0; i < opt.length; i++) {
					string = string.replaceAll("%" + (i + 1),
							msg.getString("message_variable_color") + opt[i] + msg.getString("message_color"));
				}
			}
		}
